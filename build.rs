...
fn check_and_refine_package_versions() -> anyhow::Result<()> {
    ...
    let version_from_nix = option_env!("version");

    if let Some(version_from_nix) = version_from_nix {
        if version_from_cargo.trim() != version_from_nix.trim() {
            bail!(
    ...
    let version_from_git = match run_command(git_describe) {
        ...
        Err(err) => {
            println!("{WARN}{err}");
            return Ok(()); // maybe git is not installed? exit gracefully
        }
    };
